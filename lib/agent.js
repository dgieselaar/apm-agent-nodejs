...
Agent.prototype.sendProfile = function (profileAsBuffer, metadata) {
  this._transport.sendProfile(profileAsBuffer, metadata, (err) => {
    if (err) {
      this.logger.error('Failed to send profile', err)
    }
  })
}

Agent.prototype.startCpuProfile = function (name) {
  return this._profiles.startCpuProfile(name)
}
